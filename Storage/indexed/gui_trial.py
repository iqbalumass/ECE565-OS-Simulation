...
class BlockGUI:
    ...
    def load_entries(self):
        try:
            ...
            for file_info in self.directory_data:
                ...
                self.blocks[index_block] = Block(file=file_name, next_block=None, data_blocks=data_blocks, is_index_block=True)
                
                # Mark data blocks (starting from index block's next block)
                for block in data_blocks:
                    self.blocks[block] = Block(file=file_name, next_block=None)
    ...
    def update_file(self):
        ...
        def add(file_name, start, length, position):
            ...
            self.blocks[new_block] = Block(file=file_name, next_block=None)
