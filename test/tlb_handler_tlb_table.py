...
class TLB:
    ...
    def get_tlb_table(self):
        """Return the TLB table."""
        return self.tlb_table 

def display_tlb_table(root, tlb_instance):
    """Display the TLB table using a tkinter GUI."""
    root.title("TLB Table Viewer")

    # Create the TLB Treeview Table
    tree = ttk.Treeview(root, columns=("Virtual Page", "Physical Page", "Modified", "Protection"), show="headings")
    tree.heading("Virtual Page", text="Virtual Page")
    tree.heading("Physical Page", text="Physical Page")
    ...
    # Populate the table
    for vp, (pp, mod, prot) in tlb_instance.get_tlb_table().items():
        tree.insert("", "end", values=(vp, pp, mod, prot))   
